...
import { gameWithSetupData } from 'gamesShared/helpers/testing';
...
describe('Memory Match Game Rules', () => {
  ...
  it('should change turn when two same cards are clicked if stayInTurnOnMatch is off', () => {
    client = Client({
  ...
  it('should declare player 1 as the winner if stayInTurnOnMatch is off', () => {
    client = Client({
      game: gameWithSetupData({ ...MemoryMatchGame }, undefined, { stayInTurnOnMatch: false }),
    }) as any;
    let { cards } = client.getState().G;
