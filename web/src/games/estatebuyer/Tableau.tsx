  ...
  render() {
    return (
      <div className={css.tableau}>
        {this.getDeck()}
        <div className={css.cards}>{this.getCardsOnTable()}</div>
      </div>
    );
  }

  getDeck() {
    return <DeckComponent cards={this.props.drawFrom} numCardsPerRound={this.props.numPlayers} />;
  }

  getCardsOnTable() {
    return [...this.props.cardsontable].map((card: any) => {
