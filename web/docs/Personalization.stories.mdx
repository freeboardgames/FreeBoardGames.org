...
<Meta title="Documentation/Game Personalization" />
...
**STILL IN DEVELOPMENT: Only working for AI**
...
The customization data as `GameCustommizationState` will be available at Boardgame.io's `customData` on Game's `setup` definition:
```typescript
export const FooBarGame = {
  name: 'foobar',

  setup: (ctx, customData: GameCustommizationState)) => { // <--
...
interface IBoardProps {
  ...
  playerID: string;
  gameArgs?: IGameArgs;
  customData?: GameCustommizationState; // <--
}
