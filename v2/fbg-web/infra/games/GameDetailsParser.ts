...
const parseGameInstructions = function(gameDef: any, lang:string, gameId: string) {
  const i18nGame = gameDef[lang];
  if (!('instructions' in i18nGame)) {
    throw new Error(`instructions configuration missing for ${gameId} and ${lang}`);
  }
...
const parseGameContributors = function(gameDef: any, gameId: string): string[] {
  ...
  for (const contributor of gameDef.contributors) {
      if (typeof contributor !== 'string') {
        throw new Error(`contributors usernames need to be a string for ${gameId}`); 
      }
...
export const parseGameDetails = function(gameYaml: string, lang: string, gameId: string): GameDetails {
