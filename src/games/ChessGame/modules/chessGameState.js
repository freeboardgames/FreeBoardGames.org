    ...
    for (let y=0; y<state.board.length; y++) {
        for (let x=0; x<state.board[y].length; x++) {
            ...
            if (!isCellEmpty(cell)) {
                ...
                let opponentKingCords = findCellType(stateCopy.board, 'k',
          opponentPlayer);
  ...
  //First loop will select a player piece.
    for (let y0=0; y0<state.board.length; y0++) {
        for (let x0=0; x0<state.board[y0].length; x0++) {
          ...
          getCellPlayer(selectedCell) !== player)
                continue;
...
function selectPiece(state, x, y, player) {
    ...
    case 'k': //KING
        ...
        if (y == firstRow && x == 4) { // Original position
        // Queen side
            if ((!isCellEmpty(board[y][0]) &&
              board[y][0][0] == 'r' &&
              getCellPlayer(board[y][0]) == player) &&
            isCellEmpty(board[y][1]) &&
            isCellEmpty(board[y][2]) &&
            isCellEmpty(board[y][3])) {
                toggleCellMovable(board, 2, y, player);
            }
            if ((!isCellEmpty(board[y][7]) &&
              board[y][7][0] == 'r' &&
              getCellPlayer(board[y][7]) == player) &&
            isCellEmpty(board[y][6]) &&
            isCellEmpty(board[y][5])) {
    ...
    case 'q': //QUEEN
        ...
        for (let i = 0; i < vectors.length; i++) {
            for (let delta=1; delta <= 7; delta++) {
                ...
                if (isValidCell(board, x2, y2) &&
              !isCellEmpty(board[y2][x2])) {
                    break;
                }
            }
        }
    ...
    case 'p': //PAWN
      ...
      //Left adjacent cell
        for (let delta=-1; delta<=1; delta+=2) {
            ...
            if (isValidCell(board, x2, y2) &&
            (!isCellEmpty(board[y2][x2]) ||
            isEnPassantRisk(state.board, state.enPassantRisk, x2, y2,
              y2-direction, player))) {
                toggleCellMovable(board, x2, y2, player);
            }
        }
...
function movePiece(state, x, y, player) {
  ...
  //Add En passant risk
    if (selectedCell[0] == 'p' &&
      Math.abs(selectedCellCord.y - y) == 2) {
        if (!enPassantRisk) {
            state.enPassantRisk = [];
  ...
  //Check for en passant
    if (selectedCell[0] == 'p' &&
      isEnPassantRisk(board, enPassantRisk, x, y,
        selectedCellCord.y, player)) {
        board[selectedCellCord.y][x] = '';
    }

  //Do castling
    let firstRow = 7 - 7 * player;
    if (y == firstRow && selectedCellCord.y == firstRow && selectedCell[0] == 'k'
      && selectedCellCord.x == 4) {
